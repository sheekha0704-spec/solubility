...
def get_unique_recommendations(drug_name, props):
    """Generates unique recommendations using a SHA-256 hash of the drug name."""
    LIB = {
    ...
    # Use drug name hash to ensure uniqueness even if LogP is the same
    name_hash = int(hashlib.sha256(drug_name.lower().encode()).hexdigest(), 16)
    
    results = {}
    for i, (cat, items) in enumerate(LIB.items()):
        # Seed changes for each category and each drug
...
if st.session_state.step == 1:
    ...
    with col1:
        # Assign unique key to avoid widget interaction errors
    ...
    if st.session_state.recs:
        ...
        # Using .get() to prevent KeyError if the session state is old
        status_flag = p.get('found', False)
        status_text = "✅ Data Verified" if status_flag else "⚠️ Using Estimated values"
        
        st.subheader(f"Results for {st.session_state.drug.upper()}")
        st.info(f"**Status:** {status_text} | **MW:** {p.get('MW')} | **LogP:** {p.get('LogP')}")
        
        r = st.session_state.recs
        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown("### 💧 Top Oils")
            for item in r['Oils']: st.write(f"• {item}")
        with c2:
            st.markdown("### 🧪 Top Surfactants")
            for item in r['Surfactants']: st.write(f"• {item}")
        with c3:
            st.markdown("### 🧬 Top Co-Surfactants")
...
# --- STEP 2: RADIO SELECTION ---
    ...
    st.header(f"Step 2: Component Selection for {st.session_state.drug}")
    r = st.session_state.recs
...
# --- STEP 3: ANALYTICS & PDF ---
    ...
    # Solubility Math
    logs = 0.16 - (0.63 * p.get('LogP', 3.0)) - (0.0062 * p.get('MW', 400.0))
    ee_pred = min(99.8, 70.0 + (p.get('LogP', 3.0) * 4.5))
    
    m1, m2, m3 = st.columns(3)
    m1.metric("Predicted LogS", f"{logs:.3f}")
    m2.metric("Predicted %EE", f"{ee_pred:.1f}%")
    m3.metric("System Stability", "High" if p.get('LogP', 3.0) > 2.5 else "Moderate")

    # Dynamic Bar Chart
    fig, ax = plt.subplots(figsize=(10, 4))
    features = ["LogP Impact", "MW Steric", "Lipid Affinity", "Solvent Synergy"]
    impacts = [-0.63 * p.get('LogP', 3.0)/5, -0.0062 * p.get('MW', 400.0)/100, 0.45, 0.38]
    ...
    def generate_pdf():
        ...
        pdf.cell(0, 10, f"MW: {p.get('MW')} | LogP: {p.get('LogP')}", 0, 1)
        pdf.cell(0, 10, f"Selected System: {st.session_state.sel_o} / {st.session_state.sel_s} / {st.session_state.sel_cs}", 0, 1)
    ...
    st.download_button(
        ...
        data=generate_pdf(),
        file_name=f"{st.session_state.drug}_Analysis.pdf",
