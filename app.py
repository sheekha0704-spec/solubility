...
def fetch_drug_data(drug_name):
    ...
    try:
        ...
        # Fallback to default values if API fails or drug not found
...
st.sidebar.info(f"Current Step: {st.session_state.step}")
...
if st.session_state.step == 1:
    ...
    with col2:
        ...
        if st.button("Perform AI Simulation", use_container_width=True):
            if drug_input:
                with st.spinner("Analyzing Chemical Fingerprint..."):
    ...
    if st.session_state.recs:
        st.divider()
        st.subheader(f"Recommendations for {st.session_state.drug}")
...
# --- STEP 2: RADIO SELECTION ---
    ...
    if b1.button("⬅️ Back"): 
        ...
        st.rerun()
    if b2.button("Generate Final Analysis ➡️", use_container_width=True, type="primary"):
        st.session_state.step = 3
        st.rerun()

# --- STEP 3: ANALYTICS & PDF ---
elif st.session_state.step == 3:
    st.header("Step 3: Optimization Report")
    ...
    ax.set_title(f"Solubility Drivers: {st.session_state.drug}")
    st.pyplot(fig)
    ...
    def generate_pdf():
        ...
        pdf.cell(0, 10, f"MW: {p['MW']} | LogP: {p['LogP']}", 0, 1)
        ...
        # Save image temporarily to insert into PDF
